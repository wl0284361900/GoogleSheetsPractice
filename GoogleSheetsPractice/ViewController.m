...
- (void)viewDidLoad {
    ...
    /*
     ...
     //預設的快取策略，如果快取不存在，直接從服務端獲取。
     如果快取存在，會根據response中的Cache-Control欄位判斷下一步操作，
     如: Cache-Control欄位為must-revalidata,
     則詢問服務端該資料是否有更新，無更新的話直接返回給使用者快取資料，若已更新，則請求服務端.
     
     NSURLRequestReloadIgnoringLocalCacheData = 1
     //忽略本地快取資料，直接請求服務端.
     
     NSURLRequestIgnoringLocalAndRemoteCacheData = 4
     //忽略本地快取，代理伺服器以及其他中介，直接請求源服務端.
     NSURLRequestReloadIgnoringCacheData = NSURLRequestReloadIgnoringLocalCacheData
     
     NSURLRequestReturnCacheDataElseLoad = 2
     //有快取就使用，不管其有效性(即忽略Cache-Control欄位), 無則請求服務端.
     
     NSURLRequestReturnCacheDataDontLoad = 3
     //只載入本地快取. 沒有就失敗. (確定當前無網路時使用)
